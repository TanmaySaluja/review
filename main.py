...
text = ["Solid build quality and reliable performance. Does what it claims and offers good value overall."]
rating = 5

# 1. Vectorize text
cat_pipe.transform(text)

# 2. Scale rating
rating_df = pd.DataFrame({"rating": [rating]})
num_pipe.transform(rating_df)

# 3. Combine
X_new = hstack([x_test_vec, rating_df])
